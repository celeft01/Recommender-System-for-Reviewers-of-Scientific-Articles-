...
for nam in range(len(aname63)):
    ...
    pinakas_test = df1[df1['author']==user_input].copy()
    # print(pinakas_test)
    pinakas_test2 = df2[df2['author']==user_input].copy()
    ...
    for i in range(0, len(distances.flatten())):
        ...
        if (i!=0 and (author_test.iloc[indices.flatten()[i],0] not in resList)):
    ...
    distances2, indices2 = model_knn2.kneighbors(query_item2, n_neighbors = 200)#out 10 for testing
    ...
    for i in range(0, len(distances2.flatten())):
        ...
        #else:
            #print('{0}: {1}, with distance of {2}:'.format(i, author_test.iloc[indices.flatten()[i],0], distances.flatten()[i]))
        if (i!=0 and (author_test2.iloc[indices2.flatten()[i],0] not in resList)):
